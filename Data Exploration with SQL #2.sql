...
*/
SELECT COUNT(customer_id) AS "Number of Customers",store_id 
FROM customer
WHERE active = 1
GROUP BY store_id;
/*
...
SELECT COUNT(distinct category_id) AS "Unique Categories" FROM film_category; 
/*
...
*/
SELECT MIN(replacement_cost) AS "Least Expensive", MAX(replacement_cost) AS "Most Expensive", AVG(replacement_cost) AS "Average Cost"
FROM film;
/*
...
*/
SELECT AVG(amount) as "Average Payment", MAX(amount) AS "Max Payment"
FROM payment;
/*
...
the top of the list. 
*/
SELECT customer_id, COUNT(rental_id) AS "Rentals Made"
FROM rental
